...
@section('title', __('edit') . ' - ' . __('manage_airlines'))
@section('content')
<div class="max-w-2xl">
    <div class="flex items-center mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-800 dark:text-white">{{ __('edit') }} - {{ app()->getLocale() === 'ar' ? $airline->name_ar : $airline->name_en }}</h2>
    </div>
        ...
        <form action="{{ route('admin.airlines.update', $airline) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en', $airline->name_en) }}" required class="w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    ...
                    <input type="text" name="name_ar" value="{{ old('name_ar', $airline->name_ar) }}" required class="w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $airline->code) }}" required maxlength="3" class="w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm focus:ring-2 focus:ring-primary-500 font-mono uppercase">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ app()->getLocale() === 'ar' ? 'البلد' : 'Country' }}</label>
                    <input type="text" name="country" value="{{ old('country', $airline->country) }}" class="w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ app()->getLocale() === 'ar' ? 'الشعار' : 'Logo' }}</label>
                @if($airline->logo)
                <div class="mb-2"><img src="{{ asset('storage/' . $airline->logo) }}" class="w-16 h-16 object-contain rounded"></div>
                @endif
            ...
            <div class="mb-6">
                ...
                <select name="status" class="w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm">
                    <option value="active" {{ $airline->status === 'active' ? 'selected' : '' }}>{{ __('active') }}</option>
                    <option value="inactive" {{ $airline->status === 'inactive' ? 'selected' : '' }}>{{ __('inactive') }}</option>
                </select>
            </div>
