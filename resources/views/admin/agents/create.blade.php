    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <form action="{{ route('admin.agents.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ app()->getLocale() === 'ar' ? 'المزود' : 'Provider' }} *</label><select name="provider_id" required class="w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm">@foreach($providers ?? [] as $provider)<option value="{{ $provider->id }}">{{ $provider->company_name }}</option>@endforeach</select></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div class="mb-4"><label class="flex items-center"><input type="checkbox" name="api_enabled" value="1" class="rounded text-primary-600 focus:ring-primary-500"><span class="text-sm text-gray-700 dark:text-gray-300 {{ app()->getLocale() === 'ar' ? 'mr-2' : 'ml-2' }}">{{ app()->getLocale() === 'ar' ? 'تفعيل API' : 'Enable API Access' }}</span></label></div>
