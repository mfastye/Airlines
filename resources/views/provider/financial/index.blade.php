...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 text-center"><div class="text-sm text-gray-500">{{ app()->getLocale() === 'ar' ? 'إجمالي الإيرادات' : 'Total Revenue' }}</div><div class="text-3xl font-bold text-green-600">${{ number_format($totalRevenue ?? 0, 2) }}</div></div>
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 text-center"><div class="text-sm text-gray-500">{{ app()->getLocale() === 'ar' ? 'عمولات النظام' : 'System Commissions' }}</div><div class="text-3xl font-bold text-yellow-600">${{ number_format($totalCommissions ?? 0, 2) }}</div></div>
</div>
